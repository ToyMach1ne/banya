 ...
 * Plugin class
 *
 * @package Blogclone_Ftp
 * @copyright Copyright (c) 2012, web2innovation
 */
if( !class_exists('Blogclone_Ftp') ){
class Blogclone_Ftp{
	...
	public function setMode( $mode=null ) {
		if ( !in_array( $mode, array( self::TMODE_ASCII, self::TMODE_BINARY ) ) ) {
			return $this->setError( 'transfermode has either to be Blogclone_Ftp::TMODE_ASCII or Blogclone_Ftp::TMODE_BINARY' );
		}
		$this->_mode=$mode;
		return true;
	}
	 ...
	 * the files (using Blogclone_Ftp::LS_FILES_ONLY), to list only directories (using
	 * Blogclone_Ftp::LS_DIRS_ONLY) or to show both (using Blogclone_Ftp::LS_DIRS_FILES, which is
	 * default).
	 *
	 * The 4th one is the Blogclone_Ftp::LS_RAWLIST, which returns just the array created by
	 ...
	 * Blogclone_Ftp::LS_DIRS_FILES the directories will be added above the files,
	...
	private function listAndParse( &$arrRes ) {
		...
		usort( $dirs_list, array( 'Blogclone_Ftp', 'sort' ) );
		usort( $files_list, array( 'Blogclone_Ftp', 'sort' ) );
	...
	/**
	 * последовательность всех сообщений Blogclone_Ftp
	...
	/**
	 * только ошибки полученные при исполнении Blogclone_Ftp
	 * включая все отнаследованные классы
	 *
	 * @return array
	 */
