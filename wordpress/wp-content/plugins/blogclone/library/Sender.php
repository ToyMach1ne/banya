 ...
 * Crofile class
 *
 * @package Blogclone_Sender
 * @copyright Copyright (c) 2014, web2innovation
 */
if( !class_exists('Blogclone_Sender') ){
class Blogclone_Sender {
	...
	public static function sendBackupFileAction( $_flgNotShow=false ){
		...
		include_once Blogclone::$pathName.'/library/Projects.php';
		include_once Blogclone::$pathName.'/library/Twig.php';
		include_once Blogclone::$pathName.'/library/Cron.php';
		...
		Blogclone_Twig::setOptions( $_arrProject['settings'], $_flgType );
		$_this=new Blogclone_Sender();
	...
	private function send(){
		$attachments=array( substr(ABSPATH,0,-1).self::$ds.'wp-backups'.Blogclone_Twig::_subdirCreation().Blogclone_Twig::$arrOptions['file_name'] );
		$headers='From: '.get_bloginfo('name').' <'.get_bloginfo('admin_email').'>' . "\r\n";
		$_massage=get_bloginfo('name')."\n".get_bloginfo('url')."\nbackup ".date('Y-m-d H:i:s', time() );
		if(@wp_mail(
			Blogclone_Twig::$arrOptions['send_email'],
	...
	private function sendDropbox(){
		...
		$attachments=substr(ABSPATH,0,-1).self::$ds.'wp-backups'.Blogclone_Twig::_subdirCreation().Blogclone_Twig::$arrOptions['file_name'];
		$dbxClient=Dropbox_Client(Blogclone_Twig::$arrOptions['access_token']);
		try{
			$fileInSteam=@fopen($attachments,"rb");
			if( !isset( Blogclone_Twig::$arrOptions['return']['file_size'] ) ){
				$settings['file_size']=@filesize( $attachments );
			}else{
				$settings['file_size']=(int)Blogclone_Twig::$arrOptions['return']['file_size'];
			}
			if( $fileInSteam !== false ){
				if( !isset( Blogclone_Twig::$arrOptions['return']['upload_id'] ) ){
					$data='';
					$bytesRemaining=4194304;
					while (!feof($fileInSteam) && $bytesRemaining > 0){
						$part=fread($fileInSteam, $bytesRemaining);
						if($part === false){
							if( isset( Blogclone_Twig::$arrOptions['return'] ) ){
					...
					$settings['upload_id']=Blogclone_Twig::$arrOptions['return']['upload_id'];
				}
				if( !isset( Blogclone_Twig::$arrOptions['return']['range_start'] ) ){
					$settings['range_start']=$len;
				}else{
					$settings['range_start']=(int)Blogclone_Twig::$arrOptions['return']['range_start'];
				}
				...
				$dbxClient->chunkedUploadFinish( //3
					$settings['upload_id'],
					self::$ds.str_replace( array( "http://", "https://", ".", "/" ), array( "","","_","_" ), Blogclone_Twig::$arrOptions['blog']['url'] ).self::$ds.str_replace( 'wp-backups'.Blogclone_Twig::_subdirCreation(),'','wp-backups'.Blogclone_Twig::_subdirCreation().Blogclone_Twig::$arrOptions['file_name'] ),
	...
	private function sendToFtp(){
		try{
			...
			if( is_file( substr(ABSPATH,0,-1).self::$ds.'wp-backups'.Blogclone_Twig::_subdirCreation().Blogclone_Twig::$arrOptions['file_name'] ) !== false ){
				$_object=new Blogclone_Ftp();
				$_object
					->setChmod( '0644' )
					->setHost( urldecode( Blogclone_Twig::$arrOptions['host'] ) )
					->setUser( urldecode( Blogclone_Twig::$arrOptions['user'] ) )
					->setPassw( urldecode( Blogclone_Twig::$arrOptions['pass'] ) )
					->setRoot( '/' )
					->makeConnectToRootDir();
				$_object->cd( Blogclone_Twig::$arrOptions['dir_name'] );
				$_object->fileUpload( Blogclone_Twig::$arrOptions['file_name'], substr(ABSPATH,0,-1).self::$ds.'wp-backups'.Blogclone_Twig::_subdirCreation().Blogclone_Twig::$arrOptions['file_name'] );
				$_object->closeConnection();
			}
			@unlink( substr(ABSPATH,0,-1).self::$ds.'wp-backups'.Blogclone_Twig::_subdirCreation().Blogclone_Twig::$arrOptions['file_name'] );
			return 'sended';
		}catch(Exception $e){
			return json_encode( array('error'=>$e->getMessage(),'status'=>false) );
		}
	...
	private function sendGoogle(){
		try{
			...
			if( !isset( Blogclone_Twig::$arrOptions['return']['access'] ) ){
				$client=new Google_Client();
				$client->setClientId( Blogclone_Twig::$arrOptions['google_key'] );
				$client->setClientSecret( Blogclone_Twig::$arrOptions['google_secret'] );
				$client->setRedirectUri( admin_url('admin.php?page=blogclone') );
				$client->setScopes(array('https://www.googleapis.com/auth/drive'));
				$client->refreshToken( Blogclone_Twig::$arrOptions['refresh_token'] );
				$settings=array();
				...
				Blogclone_Twig::$arrOptions['return']['access']=(array)Blogclone_Twig::$arrOptions['return']['access'];
				$settings['access']['token']=Blogclone_Twig::$arrOptions['return']['access']['token'];
				$settings['access']['expired']=Blogclone_Twig::$arrOptions['return']['access']['expired'];
			}
			if( !isset( Blogclone_Twig::$arrOptions['return']['folder_id'] ) ){
				$_folderName=str_replace( array( "http://", "https://", ".", "/" ), array( "","","_","_" ), Blogclone_Twig::$arrOptions['blog']['url'] );
				...
				$settings['folder_id']=Blogclone_Twig::$arrOptions['return']['folder_id'];
			}
			if( !isset( Blogclone_Twig::$arrOptions['return']['file_name'] ) ){
				$settings['file_name']=substr(ABSPATH,0,-1).self::$ds.'wp-backups'.Blogclone_Twig::_subdirCreation().Blogclone_Twig::$arrOptions['file_name'];
			}else{
				$settings['file_name']=Blogclone_Twig::$arrOptions['return']['file_name'];
			}
			if( !isset( Blogclone_Twig::$arrOptions['return']['file_size'] ) ){
				$settings['file_size']=@filesize( $settings['file_name'] );
			}else{
				$settings['file_size']=Blogclone_Twig::$arrOptions['return']['file_size'];
			}
			$chunkSize=256*1024*16; // dont remove 256
			if( !isset( Blogclone_Twig::$arrOptions['return']['location'] ) ){
				$_postBody=json_encode( array( 
					'title' => Blogclone_Twig::$arrOptions['file_name'],
				...
				$settings['location']=Blogclone_Twig::$arrOptions['return']['location'];
			}
			if( !isset( Blogclone_Twig::$arrOptions['return']['response_code'] ) ){
				$settings['response_code']=false;
			}else{
				$settings['response_code']=Blogclone_Twig::$arrOptions['return']['response_code'];
			}
			if( !isset( Blogclone_Twig::$arrOptions['return']['last_range'] ) ){
				$settings['last_range']=false;
			}else{
				$settings['last_range']=Blogclone_Twig::$arrOptions['return']['last_range'];
			}
			if( !isset( Blogclone_Twig::$arrOptions['return']['flg_ext_backoff'] ) ){
				$settings['flg_ext_backoff']=false;
			}else{
				$settings['flg_ext_backoff']=Blogclone_Twig::$arrOptions['return']['flg_ext_backoff'];
			}
			if( !isset( Blogclone_Twig::$arrOptions['return']['backoff_counter'] ) ){
				$settings['backoff_counter']=0;
			}else{
				$settings['backoff_counter']=Blogclone_Twig::$arrOptions['return']['backoff_counter'];
			}
			while( $settings['response_code']===false || $settings['response_code']=='308' ){
				...
				if( !isset( Blogclone_Twig::$arrOptions['return']['range_start'] ) ){
					...
					$settings['range_start']=Blogclone_Twig::$arrOptions['return']['range_start'];
				}
				if( !isset( Blogclone_Twig::$arrOptions['return']['range_end'] ) ){
					$settings['range_end']=min( $chunkSize, $settings['file_size'] - 1 );
				}else{
					$settings['range_end']=Blogclone_Twig::$arrOptions['return']['range_end'];
				}
				...
				if( $_file!='.'&&$_file!='..' ){
					if( is_dir( $_dirname.self::$ds.$_file ) ){
						...
						echo '<div class="dir_box"'.$_hidden.'>
							...
							<img class="archive_dir"'.$_addId.' src="'.Blogclone::$baseName.'skin/archive.png" rel="active" onclick="useExecuteFolder_'.$_flgType.'(this);">
							<span class="dir_name" rel="'.$_dirname.self::$ds.$_file.'" onclick="useFolderOpen_'.$_flgType.'(this);">
								<img alt="close" src="'.Blogclone::$baseName.'skin/folder.png" >&nbsp;'.$_file.'
							</span>
						</div>';
					}
				}
