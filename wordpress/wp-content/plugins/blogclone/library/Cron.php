 ...
 */
if( !class_exists('Blogclone_Cron') ){
error_reporting(E_ERROR);
ini_set('display_errors', 0);
class Blogclone_Cron {
	...
	public function backup(){
		include_once Blogclone::$pathName.'/library/PclZip.php';
		include_once Blogclone::$pathName.'/library/TarArchive.php';
		include_once Blogclone::$pathName.'/library/Backup.php';
		include_once Blogclone::$pathName.'/library/Options.php';
		include_once Blogclone::$pathName.'/library/Twig.php';
		include_once Blogclone::$pathName.'/library/Projects.php';
		include_once Blogclone::$pathName.'/library/Logger.php';
		include_once Blogclone::$pathName.'/library/Sender.php';
		...
		foreach( $arrList as &$_project ){
			...
			$_senderLog=new Blogclone_Logger( 'sender' );
			if( !empty( $this->_projectLogdate ) ){
				$_senderLog->logDate=$this->_projectLogdate;
				Blogclone_Twig::$logger->logDate=$this->_projectLogdate;
			}else{
				$_senderLog->logDate=$_project['start'];
				Blogclone_Twig::$logger->logDate=$_project['start'];
			}
			...
			if( !empty( $_senderLog->stepProject ) ){ // запускаем пошаговый отправщик
				...
				$_senderLog->stepProject['code']=json_decode( Blogclone_Sender::sendBackupFileAction( true ), true );
				...
				if( $_senderLog->stepProject['code']['status']=== 'sended' ){
					...
					$_senderLog->deleteStepLog();
					$_nextRunDate=date("U", mktime( $_project['settings']['timer']['h'], $_project['settings']['timer']['m'], 0, date("n", $_nowTime), date("j", $_nowTime), date("Y", $_nowTime) ) )+Blogclone_Cron::next( $_project['settings']['user_data']['every'] );
			...
			if( Blogclone_Twig::stepByStep( Blogclone_Twig::$logger->logDate, $_project,/*force create if no step file*/true ) ){
				$_returnArray=array();
				if( !empty( Blogclone_Twig::$logString ) ){
					$_returnArray['message']=Blogclone_Twig::$logString;
				}
				if( isset( Blogclone_Twig::$logger->stepProject['settings']['file_loader'] ) ){
					global $current_site;
					$_returnArray['file']=array(
						'name'=>Blogclone_Twig::$logger->stepProject['settings']['file_loader'], 
						'link'=>get_site_url( @$current_site->id ).'/wp-backups'.Blogclone_Twig::_subdirCreation('/').Blogclone_Twig::$logger->stepProject['settings']['file_loader'],
						'date'=>date( "Y-m-d H:i:s", @filemtime( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.'/wp-backups'.Blogclone_Twig::_subdirCreation('/').Blogclone_Twig::$logger->stepProject['settings']['file_loader'] ) )
					);
				}
				if( !empty( Blogclone_Twig::$logger->logDate ) ){
					$_returnArray['logcode']=Blogclone_Twig::$logger->logDate;
				}
				if( isset( Blogclone_Twig::$logger->stepProject['arrAllFilesCount'] ) ){
					$_returnArray['counters']=array(
						'all'=>Blogclone_Twig::$logger->stepProject['arrAllFilesCount'], 
						'now'=>Blogclone_Twig::$logger->stepProject['arrAllFilesCount']-count( Blogclone_Twig::$logger->stepProject['arrFiles'] )
					);
				}
				if( isset( Blogclone_Twig::$logger->stepProject['settings']['file_loader'] ) && !empty( Blogclone_Twig::$logger->stepProject['settings']['file_loader'] ) ){
					// создаем следующий степовый файл с отправкой
					$_senderLog->stepProject=Blogclone_Twig::$logger->stepProject;
				...
				$_returnArray['message']=Blogclone_Twig::$logString."<br/>".Blogclone_Twig::$error;
				if( !empty( Blogclone_Twig::$logger->logDate ) ){
					$_returnArray['logcode']=Blogclone_Twig::$logger->logDate;
				}
				if( isset( Blogclone_Twig::$logger->stepProject['arrAllFilesCount'] ) ){
