 ...
 * Plugin class
 *
 * @package Blogclone_Twig
 * @copyright Copyright (c) 2014, web2innovation
 */
if( !class_exists('Blogclone_Twig') ){
error_reporting(E_ERROR);
ini_set('display_errors', 0);
class Blogclone_Twig{
		...
		if( $_handle=opendir( $elementName ) ){
			while( false!==( $_file=readdir( $_handle ) ) ){
				if( $_file!='.' 
					...
					&& strpos( $elementName.self::$ds.$_file, 'blogclone'.self::$ds.'temp'.self::$ds )===false 
	...
	public static function checkSize( $arrOptions=array() ){
		...
		self::$logger=new Blogclone_Logger( 'step_log_file' );
	...
	public static function stepByStep( $logDate=false, &$arrProject=array(), $_flgForceCreateStepFile=false ){
		...
		$_dirStart=substr(ABSPATH,0,-1);
		$_pluginDir=WP_CONTENT_DIR.self::$ds.'plugins'.self::$ds.'blogclone'.self::$ds;
		$_wpContentDir=substr( WP_CONTENT_DIR, strlen( ABSPATH ) );
		$_tmpDir=getcwd();
		chdir( ABSPATH );
...
//exit;
	...
	private static function _removeOldSteps(){
		$_tempDir=WP_CONTENT_DIR.self::$ds.'plugins'.self::$ds.'blogclone'.self::$ds.'temp';
