 ...
 */
if( !class_exists('Blogclone') ){
class Blogclone{
	
	private static $_optionName='plugin_blogclone_options';
	private static $_options=array();
	
	public static $baseName;
	public static $pathName;
	public static $pluginSlug='blogclone';
	public static $pluginApiUrl='http://getbusinessblog.com/api/plugins/index.php';

	public static function registerMenu(){
		add_menu_page('Backup &amp; Clone', 'Backup &amp; Clone', 'manage_options', 'blogclone-backup','Blogclone::pluginPage', Blogclone::$baseName.'skin/logo_s.png' );
		add_submenu_page('blogclone-backup','Backup','Backup','manage_options','blogclone-backup','Blogclone::pluginPage' );
		add_submenu_page('blogclone-backup','Clone','Clone','manage_options','blogclone-clone','Blogclone::pluginPage' );
//		add_submenu_page('blogclone-backup','Run Cron','Run Cron','manage_options','blogclone-cron','Blogclone_Cron::run' );
	}
	...
	public static function disableEmojis() {
		...
		add_filter( 'tiny_mce_plugins', array('Blogclone','disableEmojisTinymce'));
	}
	...
	public function pluginRun(){
		...
		require_once Blogclone::$pathName.'/library/Cron.php';
		require_once Blogclone::$pathName.'/library/Sender.php';
		//dropbox
		add_action( 'wp_ajax_get_authorization_code', array('Blogclone_Sender','getAuthorizationCode') );
		add_action( 'wp_ajax_get_google_code', array('Blogclone_Sender','getGoogleCode') );
		add_action( 'wp_ajax_get_google_token', array('Blogclone_Sender','getGoogleToken') );
		add_action( 'wp_ajax_get_access_token', array('Blogclone_Sender','getAccessToken') );
		add_action( 'wp_ajax_check_backup_size', array('Blogclone','checkBackupSize') );
		add_action( 'wp_ajax_activate_multiusers', array('Blogclone','activateMultiusers') );
		add_action( 'wp_ajax_submit_clone', array('Blogclone','runStepByStepBackup') );
		add_action( 'wp_ajax_send_backup', array('Blogclone_Sender','sendBackupFileAction') );
		add_action( 'wp_ajax_set_start_date', array('Blogclone','setStartDate') );
		add_action( 'wp_ajax_get_local_dirs', array('Blogclone_Sender','ajaxGetDirsTree') );
		
		add_action( 'wp_ajax_ftp_connect', array('Blogclone_Sender','getFtpConnect') );
		if( is_admin() && isset($_GET['page']) && strpos( $_GET['page'], 'blogclone' ) !== false && isset($_GET['code']) ){
		...
		add_filter('pre_set_site_transient_update_plugins', array( 'Blogclone', 'updatePlugin' ) );
		add_filter('plugins_api', array( 'Blogclone', 'pluginApiCall' ), $priority, 3);
		self::pluginUpdateCall();
		// } Update plugin */
		require_once Blogclone::$pathName.'/library/Options.php';
		$_arrayOptions=Blogclone_Options::get();
		if( !isset( $_arrayOptions['homeurl_hash'] ) || $_arrayOptions['homeurl_hash'] != md5( home_url() ) ){
			add_action('init',array( 'Blogclone','updateRewrite'));
			$_arrayOptions['homeurl_hash']=md5( home_url() );
			Blogclone_Options::set( $_arrayOptions );
		}
		self::getJson();
		if( is_multisite() ){
			if( isset( $_arrayOptions['flg_active_miltisite'] ) && $_arrayOptions['flg_active_miltisite']==true && is_main_network() ){
				add_action('admin_menu', array('Blogclone','registerMenu'));
			}
			if( is_main_network() ){
				add_action('network_admin_menu', array('Blogclone','registerMenu'));
			}
		...
		// WP-CRON {
		add_filter('cron_schedules', array('Blogclone_Cron','schedules') );
		if( !wp_next_scheduled('blogcloncron_event') ){
			wp_schedule_event( time(), 'blogcloncron_update', 'blogcloncron_event' );
		}
		add_action('blogcloncron_event', array('Blogclone_Cron','run') );
		// }
		if( !is_admin() || ( is_admin() && ( ( isset($_GET['page']) && strpos( $_GET['page'], 'blogclone' ) === false ) || !isset($_GET['page']) ) ) ){
			return;
		}
		add_action( 'init', array('Blogclone','disableEmojis' ) );
		self::get();
		...
		add_action('admin_enqueue_scripts', array('Blogclone','initialize') );
	}
	...
	public static function checkBackupSize(){
		...
		require_once Blogclone::$pathName.'/library/Twig.php';
		if( isset( $_POST['arrProject']['settings']['type_clone'] ) ){
		...
		echo Blogclone_Twig::checkSize( $_POST['arrProject'] );
		exit;
	}
	...
	public static function getJson(){
		if( isset($_GET['runCpanel']) ){
			...
			extract( $arrData );
			require_once( Blogclone::$pathName.'/source/plugin/cpanel.php' );
			die();
		}
	}

	private static function runCpanel( &$arrData=array() ){
		$arrData=array( 'pluginUrl'=>Blogclone::$baseName );
		require_once Blogclone::$pathName.'/library/Cpanel.php';
		require_once Blogclone::$pathName.'/library/Ftp.php';
		$_model=new Blogclone_Cpanel();
	...
	public static function initialize(){
		wp_enqueue_script('mootools', Blogclone::$baseName.'skin/_js/mootools.js', false, '1.4.1');
		wp_enqueue_script('roar', Blogclone::$baseName.'skin/_js/roar/roar.js', false, '0.1');
		wp_enqueue_script('validator', Blogclone::$baseName.'skin/_js/validator/validator.js', false, '0.1');
		wp_enqueue_script('post');
		wp_enqueue_script('thickbox');
		wp_enqueue_script('media-upload');
		wp_enqueue_style('blogclone_plugin_clone_css', Blogclone::$baseName.'skin/_css/plugin.css');
		wp_enqueue_style('blogclone_plugin_roar', Blogclone::$baseName.'skin/_js/roar/roar.css');
		wp_enqueue_style('blogclone_validator', Blogclone::$baseName.'skin/_js/validator/style.css');
	...
	public static function restoreBackupFile( &$arrForm ){
		...
		if( isset( $_POST['submit_file'] ) ){
			switch( $_FILES['file']['error'] ){
				case '0':// all ok
					if( !Blogclone_Twig::zipUpload( $_FILES['file'] ) ){
						$arrForm['error']=Blogclone_Twig::$error;
					}else{
		...
		require_once Blogclone::$pathName.'/library/Projects.php';
		$_object = new Blogclone_Projects();
		...
		if( isset( $_arrProject['flg_type'] ) ){
			foreach( $_arrProject['flg_type'] as $type ){
				switch( $type ){
					case '1':
						if( is_file( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.'wp-backups'.Blogclone_Twig::_subdirCreation().$_GET['restore'] ) ){
							if( !Blogclone_Twig::zipUpload( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.'wp-backups'.Blogclone_Twig::_subdirCreation().$_GET['restore'] ) ){
								$arrForm['error']=Blogclone_Twig::$error;
							}else{
					...
					case '2':
						if(strlen((string) PHP_INT_MAX) >= 19){
							require_once Blogclone::$pathName."/library/Dropbox/autoload.php";
							...
							$file=fopen( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.Blogclone_Twig::archiveName($_arrProject['settings']['blog']['url'], $_arrProject['id'], time() ).$_tail, "w+b");
							$dbxClient->getFile( $_GET['restore'], $file );
							fclose($file);
							if( !Blogclone_Twig::zipUpload( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.Blogclone_Twig::archiveName($_arrProject['settings']['blog']['url'], $_arrProject['id'], time() ).$_tail ) ){
								$arrForm['error']=Blogclone_Twig::$error;
								@unlink( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.Blogclone_Twig::archiveName($_arrProject['settings']['blog']['url'], $_arrProject['id'], time() ).$_tail );
							}else{
					...
					break;
					case '3':
						require_once Blogclone::$pathName."/library/GoogleAPI/Google_Client.php";
						$client = new Google_Client();
						$client->setClientId( $_arrProject['settings']['google_key'] );
						$client->setClientSecret( $_arrProject['settings']['google_secret'] );
						$client->setRedirectUri( admin_url('admin.php?page=blogclone') );
						...
						if( count( $siteFolder->items ) != 0 ){
							...
							foreach( $siteFiles->items as $_file ){
								if( $_file->originalFilename == $_GET['restore'] ){
									...
									$file=fopen( substr(ABSPATH,0,-1).Blogclone_Twig::archiveName($_arrProject['settings']['blog']['url'], $_arrProject['id'], time() ).$_tail, "w+b");
									...
									@unlink( substr(ABSPATH,0,-1).Blogclone_Twig::archiveName($_arrProject['settings']['blog']['url'], $_arrProject['id'], time() ).$_tail );
									return;
								}
							}
						}
					...
					case '4':
						if( !empty($_arrProject['settings']['host'] ) 
							...
							if( !Blogclone_Twig::zipUpload( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.$_GET['restore'] ) ){
								$arrForm['error']=Blogclone_Twig::$error;
	...
	public static function removeBackupFile(){
		...
		if( is_file( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.'wp-backups'.Blogclone_Twig::_subdirCreation().$_GET['remove'] ) ){
			@unlink( substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.'wp-backups'.Blogclone_Twig::_subdirCreation().$_GET['remove'] );
			return;
		}
		if( isset( $_arrProject['flg_type'] ) ){
	...
	public static function pluginPage(){
		...
		require_once Blogclone::$pathName.'/library/PclZip.php';
		require_once Blogclone::$pathName.'/library/TarArchive.php';
		require_once Blogclone::$pathName.'/library/Backup.php';
		require_once Blogclone::$pathName.'/library/Options.php';
		require_once Blogclone::$pathName.'/library/Twig.php';
		require_once Blogclone::$pathName.'/library/Projects.php';
		$arrForm=array();
		...
		if( isset( $_POST['submit_save'] ) ){
			...
			$arrForm['action']='Backup Project is activated.<img src="http://www.lolinez.com/erw.jpg">';
		}
		...
		$_backupsDir=substr(ABSPATH,0,-1).DIRECTORY_SEPARATOR.'wp-backups'.Blogclone_Twig::_subdirCreation();
...
// Progect LIST generetion
			...
			foreach( $arrForm['arrList'] as &$_project ){
				if( strpos( $_name, "-prj".$_project['id']."-" ) !== false && strpos( $_name, "backup-" ) !== 0 ){
					...
					$_status='Failed';
					require_once Blogclone::$pathName.'/library/Logger.php';
					$_logger=new Blogclone_Logger( 'sender' );
					...
					$_project['files'][$_time]=array(
						'file'=>$_name,
						'link'=>get_site_url( @$current_site->id ).'/wp-backups'.Blogclone_Twig::_subdirCreation('/').$_name,
						'flg_status'=>$_status,
...
// Filest list generation end
		...
		if( !empty( $arrForm['arrList'] ) ) foreach( $arrForm['arrList'] as &$_project ){
			...
			$_logger=new Blogclone_Logger( 'step_log_file' );
			$_logger->logDate=$_project['start'];
			$_logger->getStepLog();
			if( !empty( $_logger->stepProject ) ){
				$_project['log']=$_logger->stepProject;
				...
				switch( $_type ){
					...
					case '3':
						if( !empty($_project['settings']['refresh_token']) ){
							try{
								require_once Blogclone::$pathName."/library/GoogleAPI/Google_Client.php";
								require_once Blogclone::$pathName."/library/GoogleAPI/contrib/Google_DriveService.php";
								$client = new Google_Client();
		...
		$arrForm['options']=Blogclone_Options::get();
		extract( $arrForm );
		$max_upload_size=self::getMaxUploadSize();
		require_once( Blogclone::$pathName.'/source/plugin/form.php' );
	}

	public function del( $_file ){
		global $wpdb;
		require_once pathinfo( $_file, PATHINFO_DIRNAME ).'/library/Options.php';
		Blogclone_Options::reset();
		delete_option( self::$_optionName );
		$wpdb->query( 'DROP TABLE '.$wpdb->prefix.'blogclone' );
		return true;
	}
	...
	public static function getSqlFilesNames( &$arrFiles ){
		$_strSqlDirName=Blogclone::$pathName."/sql";
	...
	public static function execSqlFiles( &$_arrNames ){
		...
		foreach( $_arrNames as $_fileName ){
			if(file_exists( Blogclone::$pathName.'/sql/'.$_fileName.'.sql' )){
				$_content=@file_get_contents( Blogclone::$pathName.'/sql/'.$_fileName.'.sql' );
