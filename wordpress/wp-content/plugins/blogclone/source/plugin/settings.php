...
<li>
	<label><?php if( $_GET['page'] == 'blogclone-backup' ){ ?>Backup<?php }else{ ?>Clone<?php } ?> Every</label>
...
<li>
	...
	<p class="helper"><font color="Red">Note:</font> Use this option to <?php if( $_GET['page'] == 'blogclone-backup' ){ ?>backup<?php }else{ ?>clone<?php } ?> your database, themes, plugins, and media uploads. Lets you <?php if( $_GET['page'] == 'blogclone-backup' ){ ?>backup<?php }else{ ?>clone<?php } ?> your custom data excluding default WordPress engine files.</p>
</li>
	...
	$('only_settings_backup_<?php echo $flg_type;?>').addEvent('change',function(){
		if( this.checked ){
			...
			$$('.archive_dir').each(function(elt){
				...
				elt.set('rel','active');
				elt.set('src','<?php echo Blogclone::$baseName;?>skin/archive.png');
		...
		if( $('only_settings_backup_<?php echo $flg_type;?>').get('checked') == true ){
			$$('.archive_dir').each(function(elt){
				if( !( elt.get('id') == 'this_content' && ($('check_plugins').get('checked') == true || $('check_thems').get('checked') == true || $('check_uploads').get('checked') == true) ) ){
					elt.set('rel','none');
					elt.set('src','<?php echo Blogclone::$baseName;?>skin/other.png');
					if( elt.getNext().getNext('.dir_list') != undefined ){
						elt.getNext().getNext('.dir_list').destroy();
					}
					elt.getPrevious().set('value','1');
			...
			if( $('check_db').get('checked') == true ){
				...
				$('this_db').set('src','<?php echo Blogclone::$baseName;?>skin/archive.png');
			...
			if( $('check_plugins').get('checked') == true || $('check_thems').get('checked') == true || $('check_uploads').get('checked') == true ){
				...
				$('this_content').set('src','<?php echo Blogclone::$baseName;?>skin/archive.png');
